...
### 문제 상황: 코드 중복과 비효율성
```
// 과일별로 메서드를 각각 만들어야 하는 중복
private List<Fruite> findApples(List<Fruite> fruits) {
...
private List<Fruite> findBananas(List<Fruite> fruits) { ... }
// 새로운 조건마다 메서드 추가 필요
```
...
```
- 잠점으로는 무한한 조건 추가가 가능하고 단점으로는 복잡하고 장황해진다

### 3단계 변화: Lambda (Java 8+) + 표준 함수형 인터페이스 활용
```
// 익명 클래스는 람다로 간소화
filterFruitsLambda(fruits, fruit -> fruit.getName().equals("사과"));
filterFruitsLambda(fruits, fruit -> fruit.gerPrice() > 1_000);

private static List<Fruit> filterFruitsLambda(List<Fruit> fruits, Predicate<Fruit> filter) {
    List<Fruit> results = new ArrayList<>();
    for (Fruit fruit : fruits) {
        if (filter.test(fruit)) { // test() 메서드 사용
            results.add(fruit);
        }
    }
    return results;
...
### 4단계 변화: Stream API + 메서드 참조
```
// Lambda 사용
filterFruitsStream(fruits, fruit -> fruit.getName().equals("사과"));

// 메서드 참조 활용
filterFruitsStream(fruits, JavaFruit::isApple); // fruit -> fruit.isApple()와 동일하지만 더 간결

// 더욱 간결한 함수형 스타일
private static List<JavaFruit> filterFruitsStream(List<JavaFruit> fruits, Predicate<JavaFruit> filter) {
    return fruits.stream()
            .filter(filter)
            .collect(Collectors.toList()); // toList()는 Java 16 +
}
```
...
// 불가능한 것들
// 1. 함수를 변수에 직접 할당
// Function myFunction = someMethod;

// 함수를 직접 파라미터로 전달
// processData(someMethod);

// 가능한 것: 함수형 인터페이스를 우회
Predicate<Fruit> filter = fruit -> fruit.getName().equals("사과");
```
...
// 실제: Predicate 인터페이스의 구현체를 전달
filterFruitsStream(fruits, new Predicate<Fruit>() {
    @Override
...
```
- Java는 함수를 2급 시민으로 다루지만, 함수형 인터페이스와 람다를 통해 함수형 프로그래밍의 이점을 제공

## 코틀린에서의 람다

### Java vs Kotlin의 근본적 차이
- Java 함수는 2급 시민으로 인터페이스를 통해서만 전달 가능
- Kotlin 함수는 1급 시민으로 변수 할당, 파라미터 전달 직접 가능
...
```
/// 함수 타입: (파라미터 타입...) -> 반환 타입
val filter: (Fruit) -> Boolean = { fruit -> fruit.name == "사과" }
val converter: (String) -> Int = {str -> str.length }
val multiParam: (String, Int, Boolean) -> String = { a, b, c -> "$a-$b-$c" }
```
...
private static List<Fruit> filterFruits (
    List<Fruit> fruits,
    Predicate<Fruit> filter // 인터페이스
) { ... }

// Kotlin: 함수 타입 직접 사용
private fun filterFruits(
    fruits: List<Fruit>,
    filter: (Fruit) -> Boolean // 함수 타입 자체
): List<Fruit> {
    ...
}
```

### Kotlin Lambda 문법 간소화
...
#### it 키워드 사용
```
// 파라미터가 1개인 경우 it으로 간소화
fruits.filter { it.name == "사과" }
fruits.map { it.price * 0.9 }
fruits.any { it.price > 5_000 }

// 파라미터가 여러 개면 명시적 선언
fruits.groupBy { fruit -> fruit.name } // it 사용 불가
```
...
#### 여러 줄 Lambda
```
val complexFilter = { fruit: Fruit ->
    val isTargetFruit = fruit.name in listOf("사과", "바나나")
    val isReasonablePrice = fruit.price <= 3000
    val isPopular = fruit.name == "사과"
    
    isTargetFruit && isReasonablePrice && isPopular // 마지막 줄이 반화 값
}

filterFruits(fruits, complexFilter)
```

#### 고차 함수 활용
```
// 함수를 반환하는 함수
fun createPriceFilter(maxPrice: Int): (Fruit) -> Boolean {
    return { fruit -> fruit.price <= maxPrice }
}

// 사용
val cheapFilter = createPriceFilter(2_000)
filterFruits(fruits, cheapFilter)
```
...
// 다양한 고차 함수들
fruits.any { it.price > 5_000 } // 하나라도 조건 만족? 
fruits.all { it.price > 0 } // 모두 조건 만족?
fruits.count { it.name == "바나나" } // 조건 만족하는 개수
fruits.groupBy { it.name } // 이름별로 그룹핑
fruits.partition { it.price > 2_000 } // 조건으로 분할
```
- Kotlin의 lambda는 함수를 1급 시민으로 다루어 Java보다 간결하고 표현력 있는 함수형 프로그래밍을 가능하게 한다

## Closure
### Java의 제약: Effectively Final
```
...
// Java 람다는 변경 불가능한 변수만 캡처할 수 있다

// 해결 방법 1: final 변수 사용
final String finalTarget = "수박"
filterFruits(fruits, (fruit) -> targetFruitName.equals(fruit.getName());

// 해결 방법 2: effectively final (한 번만 할당)
String finalTarget = "수박"
filterFruits(fruits, (fruit) -> targetFruitName.equals(fruit.getName());
```
...
targetFruitName = "수박"
filterFruits(fruits) { it.name == targetFruitName } // 정상 동작
```

- 람다를 씀과 동시에 만들어두었던 targetFruitName을 사용하는 코드
- Variable used in lambda expression should be final or effectively final
- 람다 표현에 사용되는 변수는 최종 변수 또는 사실상 최종 변수여야 합니다
- 람다를 사용할 때 람다 밖에 있는 변수를 사용하는 경우 제약이 있을 수 있다.
- final인 변수 혹은 실질적으로 final인 변수만 사용할 수 있다. 이 경우에는 바나나였다가 수박으로 한 번 바뀌었기 떄문에 사용이 불가능하다
...
### Closure
Closure는 Lambda가 실행되는 시점에 참조하는 외부 변수들을 모두 초획(capture)한 데이터 구조이다

#### Closure의 동작 원리
```
fun createFilter(): (Fruit) -> Boolean {
    var count = 0 // 지역 변수
...
fruits.filter(filter) // count 변수를 계속 유지하고 수정
```
...
fun demonstrateClosure() {
    ...
    val lambda = { value: Int ->
        capturedVar = "변경됨" // 외부 변수 수정
        println("포획된 값: $capturedVar")
        value * multiplier
    }
    
    // lambda는 capturedVar와 multiplier를 "포획"하고 있음
    lambda(5)
    println(capturedVar) // "변경됨" - 람다에서 수정한 값이 반영
}
```
- Closure는 람다가 외부 스코프의 변수들을 포획하여 자신만의 환경을 구성하는 것
- JavaScript나 PHP에서는 각종 버그가 되는 문제의 원인이 되는 이야기를 들은 기억도 나는데 Kotlin은 타입 안정성과 JVM의 GC 덕분에 상대적으로 안전하다고 한다
...
//      ↑       ↑            ↑     ↑     ↑               ↑
//    inline  제네릭타입     확장함수 배개변수 람다타입         반환타입
```
- inline: 성능 최적화를 위한 인라인 함수
- <T: Closeable?, R>: T는 nullable Closeable, R은 반환 타입
- T.use: Closeable을 구현하는 타입 T에 대한 확장함수
- block: (T) -> R: T를 받아 R을 반환하는 람다 함수
- Kotlin의 use 함수는 자원 관리를 자동화하면서도 함수형 프로그래밍의 장점을 살린 우아한 해결책이다
...
## 중첩된 컬렉션 처리
- NestingCollections.kt

# 19. 코틀린의 이모저모

## Type Alias
- 복잡하거나 긴 타입에 별칭을 부여하여 코드 가독성과 유지보수성을 향상시키는 기능
...
```
// 불필요한 typealias
typealias MyString = String // 의미없음
typealias MyInt = Int // 불필요
...
```
// 의미 전달이 명확한 이름 사용
typealias ValidationResult = Result<Unit, String>
typealias DatabaseConnection = Connection
typealias JsonResponse = Map<String, Any?>
```

- 가독성 향상: 복잡한 타입을 의미있는 이름으로 표현
- 유지보수성: 타입 변경 시 typealias 한 곳만 수정
- 코드 간소화: 긴 제네릭 타입을 짧게 축약
- 의미 전달: 타입의 역할과 목적을 이름으로 명확히 표현
- typealias는 복잡한 타입을 단순화하고 의미를 명확하게 전달해준다

## as import
- 서로 다른 패키지에 같은 이름의 함수나 클래스가 있을 때 동시에 import하면 이름 충돌이 발생한다
- 이 때 as를 사용한 Import Alias를 사용하면 해결된다

`import package.functionName as newName`
...
// Import Alias를 사용해 이름 충돌 해결
import etc.kotlin.a.printHelloWorld as printHelloWorldA
import etc.kotlin.b.printHelloWorld as printHelloWorldB
...
```
// 의미 없는 축약
import com.payment.PaymenrService as PS
import com.user.UserService as US

// 의미 전달이 명확한 이름
import com.payment.PaymenrService as PaymentProcessor
import com.user.UserService as UserManabger
```

- 출처를 나타내는 접두사
```
// 라이브러리 출처를 나타내는 접두사 사용
import retrofit2.Response as Retrofit2Response
import okhttp3.Response as OkHttp2Response
import java.net.http.HttpResponse.Response as JavaHttpResponse
```

- import alias는 이름 충돌을 해결하고 코드 가독성을 향상 시키지만 의미 있는 이름을 사용하여 코드를 명확히 전달하는 것이 중요하다

## 구조분해와 ComponentN 함수
- 구조분해란 복합적인 값을 분해하여 여러 변수를 한 번에 초기화하는 기능이다
...
// 구조분해 사용
val (name, age) = person
println("이름은 ${name}, 나이는 ${age}") // 이름은 혁, 나이는: 100
```

### ComponentN 함수의 실체
```
// 구조분해는 내부적으로 componentN 함수를 호출
val name = person.component1()  // person.name과 동일
val age = person.component2() // person.age와 동일
...
val (age, name) = person
println("이름은 ${name}, 나이는 ${age}") // 이름은 100, 나이는: 혁
```

### 일반 클래스에서 구조분해 구현 가능 - ComponentN 함수를 직접 구현
```
class Person(
    val name: String,
    val age: Int
...
```
- 일반 클래스에서 componentN 함수를 하용하면 `Destructuring of type 'Person' requires operator function 'component1()'`에러 발생
...
// Data class는 최대 component5까지 자동 생성
val user = User("홍홍홍", 100, "hong@homg.com", "010-1234-5678")
val (name, age, email, phone) = user // component1~4사용
```

- 부분적 구조분해
```
// 필요한 부분만 추출
val (name, age) = person // component1, 2만 사용
val (name, _, email) = user / component1, 3만 사용 (age는 무시)
```
...
// Map.Entry가 component1(key), component2(value) 제공
```
...
// 헷갈리기 쉬운 사용
val (y, x) = Point(10, 20) // x = 20, y = 10 (의도와 반대로 설정)
...
val (n, a, d, s, p e) e employee // 어떤 값이 무엇인지 불분명
```
- 구조분해는 순서 기반으로 동작하므로 변수 순서를 정확히 맞춰야 하며, 읨가 명확할 때만 사용하는 것이 좋다

## Jump와 Label
- 기본 제어 흐름 키워드의 세 가지 핵심 키워드
  - return: 가장 가까운 함수에서 값을 반환
  - break: 가장 가까운 루프를 종료
  - continue: 가장 가까운 루프의 다음 반복으로 건너뜀
- Java와 동일하게 동작하지만, 함수형 프로그래밍에서 약간의 차이가 있다
...
run {
    numbers.forEach { number ->
        if (number == 2) {
            return@run // break 역할 - forEach 종료
        }
...
numbers.forEach { number ->
    if (number == 2) {
        return@forEach // continue 역할 - 현재 반복만 건너뜀
    }
...
loop@ for (i in 1..100) {
    for (j in 1..100) {
        if (j == 2) {
            break@loop // 외부 loop 종료
        }
        println("${i} ${j}") // "1 1" 만 출력되고 완전 종료
    }
}
```
...
outer@ for (i in 1..3) {
    for (j in 1..3) {
        if (j == 2) {
            continue@outer // 외부 루프와 다음 i로 이동
        }
        println("$i $j")
    }
}
...
```
- Label 사용을 피해야 하는 이유
  - 가독성 저하: 코드 흐름을 따가가기 어려움
  - 복잡도 증가: 디버깅과 유지보수 어려움
  - 실수 가능성: 잘못된 Label 참조로 인한 버그
  - 대안 존재: 대부분 다른 방법으로 해결 가능
- 코드의 가독성과 유지보수성을 위해 복잡한 제어 흐름보다는 함수 분리와 함수형 접근을 고려하는 것이 더 나은 선택

## TakeIf와 TakeUnless
- 조건에 따라 값 또는 null을 반환하는 간결한 방법을 제공하는 함수들이다
- takeIf: 조건이 참이면 값, 거짓이면 null 반환
- takeUnless: 조건이 거짓이면 값, 참이면 null 반환
...
fun getNumberOrNullV1(): Int? {
    return if (number <= 0) { // 0 보다 크면 number, 아니면 null
        null
    } else {
        number
    }
...
fun getNumberOrNullV2(): Int? {
    ...
    return number.takeIf { number -> number > 0 } // 조건이 참이면 number, 거짓이면 null
}
...
fun getNumberOrNullV3(): Int? {
    // return number.takeIf { it <= 0 }
    return number.takeUnless { number -> number <= 0 } // 조건이 거짓이면 number, 참이면 null
}
```
- takeIf와 takeUnless는 null 처리를 간결하게 만들어 주는 ㅎ마수형 프로그래밍 스타일의 체이닝을 가능하게 해준다
...
- 일시적인 영역을 형성하는 함수로, 람다를 통해 코드를 간결하게 만들거나 메서드 체이닝에 활용된다
- scope function은 람다를 활용해 임시 영역을 생성하여 코드를 더 간결하고 안전헤가 만들어 준다
...
// let으로 개선 (Safe Call과 조합)
fun printPersonLet(person: Person?) {
    person?.let { // person이 null이 아닐 때문 실행
...
```
- 동작원리
  - 확장함수: 모든 타입 T에서 호출 가능
  - 파라미터: (T) -> R 타입의 람다 함수
  - 반환값: 람다 실행 결과 R
  - it 사용: 람다 내에서 호출 객체를 it으로 접근
...
```
// let -> it 사용
val letValue = person.let { it.age } // age 반환

// run - this 사용 (생략 가능)
val runValue = person.run { age } // age 반환
```

- 객체 자체 반환
```
// also - it 사용
val alsoValue = person.also { it.age } // person 반환

// apply - this 사용 (생략 가능)
val applyValue = person.apply { age } // person 반환
```
...
// this - 생략 가능
person.run { age } // this.age와 동일
person.apply { age } // this.age와 동일
```
...
val person = Person("혁", 100)
with(person) {
    println(name)  // this  생략
    println(this.age) // this 명시
}
```
...
strings.map { is.length }
    .filter { it > 3 }
    .let(::println)
```
- 함수 체이닝의 최정 결과를 다른 함수로 전달해야 할 때 사용한다
...
```
- null이 아닌 값에 대해서만 코드 블록을 실행하고 싶을 때 사용한다. Safe call(?.)과 함께 자주 활용된다
...
```
- 제한된 영역에서만 사용할 임시 변수를 반들 때 사용한다. 불필요한 변수 선언을 피할 수 있다

- 핵심 패턴
  - 체이닝 연결: 결과를 다음 단계로 전달
  - Null 안정성: ?.let으로 안전한 실행
  - 스코프 제한: 임시 변수의 생명주기 관리
- let은 람다 결과를 반환하므로 값 변환이 필요한 경우에 적합하다

#### 함수 분리가 더 나은 경우
```
// 복잡한 로직 - 함수 분리 권장
val result = data.first().let { item ->
    val proccessed = complexProcessingLogic(item)
    val validated = validateAndTransform(proccessed)
    val formatted = applyBusinessRules(validated)
    formatted.uppercase()
}

// 개선: 함수로 분리
val result = processFirstItem(data.first())
```
- 복잡하 비즈니스 로직이나 여러 단계의 변환이 필요한 경우 함수 분리가 적절하다

#### let이 더 적절한 경우
```
// 간단한 변환 - let이 오히려 직관적
val displayText = user.name.let {
    if (it.length > 10) "${it.take(7)}..." else it
}

// 함수로 분리하면 오히려 복잡해짐
private fun truncateIfNeeded(name: String): String = 
    if (it.length > 10) "${it.take(7)}..." else it
```
- 핵심 판단 기준
  - 간단한 변환: let 사용이 더 직관적
  - 복잡한 로직: 함수 분리가 가독성과 테스트 용이성에 유리
  - 재사용성: 다른 곳에서도 쓸 로직이면 함수로 분리
- "항상 함수로 분리해야 한다"는 절대적 규칙보다는 로직의 복잡도와 맥락을 고려한 유연한 접근이 필요하다

### run 사용시점 - 객체 초기화와 반환값 계산
- 객체 초기화와 반환 값의 계산을 동시에 해야 할 때 사용된다

#### 메서드 레퍼런스 활용
```
val person = Person("혁", 100).run(personRepository::save)
```
- 객체를 생성하고 즉시 저장하여 저장된 객체를 반환받을 때 사용한다

#### 추가 설정과 함께
```
val person = Person("혁", 100).run( {
    boddy = "독서"
    personRepository.save(this)
}
```
- 생성 후 추가 프로퍼티를 설정하고 저장하는 패턴이다

#### java 방식과 비교
```
// Java로 오래 개발한 개발자들에게 익숙한 방식
val person = personRepository.save(Person("혁", 100))

// run을 사용한 방식 (가독성 관점에서 호불호 존재)
val person = Person("혁", 100).run(personRepository::save)
```
- 사용 판단 기준
  - 반복되는 후처리: 생성자나 init 블록으로 해결 고려
  - 긴 생성 로직: run을 사용하면 가독성 향상 가능
  - 코드 스타일: 팀 컨벤션과 가독성을 고려하여 선택
- run은 객체 생성과 즉시 처리를 체이닝할 때 유용하지만, 과도한 사용보다는 상황에 맞는 적절한 선택이 중요하다

## apply - 객체 설정과 반환
- 객체 자체를 반환하면서 설정을 변경할 떄 사용한다
...
```
- 생성자에 없는 프로퍼티를 추가로 설정해야 하는 테스트 객체 생서엥 유용하다
...
### 두 번째 패턴을 잘 사용하지 않는 이유
- 불변성 위배: 객체 상태 변경은 함수형 프로그래밍 원칙에 반한다
- 부작용 발생: growOld() 같은 상태 변경 함수는 예측하기 어려운 부작용을 야기한다
- 가독성 저하: Call Chain 중간의 상태 변경은 코드 흐름을 이해하기 어렵게 만든다
- 디버깅 어려움: 언제 어디서 개체가 변경되었는지 추적하기가 복잡하다
- apply는 불변 객체의 초기 설정에만 사용하고, 상태 변경이 필요함녀 새 객체를 생성하여 반환하는 방식을 선호한다


## also 사용 시점 - 객체 자체 반환과 부수 효과
- 객체 자체를 반환하면서 부수 효과(로깅, 디버깅 등)를 수행할 때 사용한다
```
mutableListOf("one", "two", "three")
    .also { println("The list elements before adding new one: $it") }
    // .add("four") Boolean 반환으로 체이닝 어려움
    .also { it.add("four") } // also 내부에서 수정
```
...
- 주요 용도
  - 로깅 / 디버깅: 객체 상태 확인
  - 부수 효과: 객체 반환은 유지하면서 추가 작업 수행
- also는 객체 수정이 아닌 부수 효과 수행이 주목적이다. 객체 수정이 목적이면 apply를 사용하는 것이 더 적절하다

## with 사용 시점 - 객체 변환 시 코드 간소화
- 일반 함수로, 객체의 여러 프로퍼티를 참조하여 다른 객체로 변환할 때 사용한다
```
return with(person) {
    PersonDto(
        name = name, // this.name (생략)
        age = age // this.age (생략)
    )
}
...
```
- 주요 특징
  - this 생량 가능: 반복적인 객체 참조 제거
  - 확장함수 아님: with(객체) { } 형태로 사용
  - 변환 로직: 한 객체에서 다른 객체로 매핑할 떄 유용

- 사용 상황 - 정적 팩토리나 확장함수를 만들기 어려운 경우 
  - 모둘 간 의존성 문제
  - 임시적인 변환 로직
  - 여러 필드 참조가 필요한 객체 생성
- 동일 객체의 여러프로퍼티를 반복 참조할 때 코드 중복을 줄이는 용도로 활용한다
...
### 1번 코드가 더 나을 수 있는 이유
- 학습 곡선과 이해도
  - 숙련도 의존: 2번은 Kotlin에 익숙한 개발자만 빠르게 이해 가능
  - 복잡성 증가: takeIf, let, 메서드 레퍼런스 등 여러 개념이 한 번에 사용됨
- 개발 생산성
  - 디버깅 용이성: 전통적인 if-else는 IDE 지원과 디버깅이 더 쉬움
  - 수정 용이성: 추가 요구사항 발생 시 어디에 로직을 넣을지 명확함

### 숨겨진 버그 위험
```
// 위험한 상황: showPerson이 null 반환 시
person?.takeIf { it.isAdult }
    ?.let(view::showPerson) // null 반환하면
    ?: view.showError() // 이 부분도 실행된다
```

### Scope Function이 항상 좋은 가독성을 보장하지 않는다
- 사용 빈도가 낮은 관용구: 오히려 복잡성 증가
- 적절한 컨벤션 필요: 팀의 Kotlin 숙련도와 선호도 고려
- 과도한 체이닝 지양: 한 문장에 여러 scope function 사용 시 복잡도 급중

- Scope function은 도구일 뿐, 상황에 맞는 적절한 선택이 가독성과 유지보수성을 결정한다